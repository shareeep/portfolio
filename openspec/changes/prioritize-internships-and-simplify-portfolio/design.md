...
The GovTech entry will adapt the settled resume rather than duplicate it verbatim. It will state ownership across two AI products and group the Langfuse, prompt, and document-processing work into readable evidence. GovTech and HTX will both use implied-first-person, action-led resume phrasing so the section does not change grammatical perspective between entries. The website will preserve the resume's dates, organization, role, and completed-work claims.
