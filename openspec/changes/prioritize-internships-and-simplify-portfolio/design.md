...
The GovTech entry will adapt the settled resume rather than duplicate it verbatim. It will state ownership across two AI products, say that Shariff built the document-job queue system, and connect the prompt changes to improved cache reuse and reduced latency. GovTech and HTX will both use implied-first-person, action-led resume phrasing so the section does not change grammatical perspective between entries. The website will preserve the resume's dates, organization, role, and completed-work claims.
...
The public SMU BIA story will stay focused on two connected parts: the embedding and traditional-ML benchmark, then the Singlish guardrail extension. It will not use numbered track headings. The removed sarcasm-aware multi-task model, local-LLM workflow, and KG-RAG experiment will also be removed from frontmatter highlights and tags, datasets, and the technology list so the metadata matches the body.
